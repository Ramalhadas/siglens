...
document.querySelectorAll('.rows-number-option').forEach(option => {
  option.addEventListener('click', function() {
    ...
    currentPage = 1; // Reset to first page whenever rows per page is changed
    console.log(`Selected rows per page: ${rowsPerPage}`);
    let data = getSearchFilter(false, false); // Ensure this function includes all necessary parameters
...
function updatePaginationControls() {
   ...
   console.log(`Updating pagination controls: currentPage=${currentPage}, totalRows=${totalRows}, rowsPerPage=${rowsPerPage}, totalPages=${totalPages}`);

   prevPageBtn.disabled = currentPage === 1;
   nextPageBtn.disabled = currentPage >= totalPages;

   console.log(`Prev button disabled: ${prevPageBtn.disabled}`);
...
function doSearch(data) {
  // Add the default or existing parameters if they are not already in data
  data.searchText = data.searchText || '*';
  data.startEpoch = data.startEpoch || 'now-15m'; // Replace with your default value
  data.endEpoch = data.endEpoch || 'now'; // Replace with your default value
  data.indexName = data.indexName || '*'; // Replace with your default value
  data.queryLanguage = data.queryLanguage || 'Splunk QL'; // Replace with your default value

  console.log(`Sending search request:`, data);

  // Calculate 'from' based on 'page' and 'rowsPerPage'
  if (data.page !== undefined && data.rowsPerPage !== undefined) {
      data.from = (data.page - 1) * data.rowsPerPage;
  }
  ...
  socket.onmessage = function (event) {
      ...
      switch (eventType) {
          ...
          case "COMPLETE":
              ...
              console.log(`Updating totalRows: ${totalRows}`);
              updatePaginationControls();
              break;
          case "TIMEOUT":
